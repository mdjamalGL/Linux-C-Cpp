...
int main(int argc, char *argv[]) {
  ...
  data.audioConvert = gst_element_factory_make ("audioconvert", "aconvert");
  data.videoConvert = gst_element_factory_make ("videoconvert", "vconvert");
  data.resample = gst_element_factory_make ("audioresample", "resample");
  data.audioSink = gst_element_factory_make ("autoaudiosink", "asink");
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  if (g_str_has_prefix (new_pad_type, "audio/x-raw")) {
    ...
    /* If our audioConverter is already linked, we have nothing to do here */
  ...
  else if(g_str_has_prefix (new_pad_type, "video/x-raw"))
  {
    ...
    /* If our videoConverter is already linked, we have nothing to do here */
    if (gst_pad_is_linked (sink_pad)) {
  ...
  {
    g_print ("It has type '%s' with is neither (audio/x-raw | video/x-raw), Ignoring\n", new_pad_type);
    goto exit;
  }
